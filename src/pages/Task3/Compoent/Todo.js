...
const Todo=()=>{
  ...
  const {task,HandleChange,HandleClick,AllTask,HandleActive,HandleCompleted}=useGlobalContext();
...
{AllTask.length===0 ? <h1 className="enter-task">Plese enter task !!!!!</h1>:<TodoList/>} 
  </div>
  <hr/>
...
{AllTask.length===0 ? <h1 className="count-task">No task....</h1>:<h2 className="count-task">{AllTask.length} items left</h2>} 
