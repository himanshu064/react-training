  ...
  return (
   ...
   {/* All Buttons */}
    <div className='d-flex justify-content-end mt-4 mb-3 page-btn'>
        ...
        <button disabled={skip===0} className="btn1" onClick={() => setSkip(skip - limit)} > Previous </button>
           <Pagination postperpage={limit} totalPosts={total} paginate={paginate}/>
        <button  className="btn2" onClick={() => setSkip (parseInt (skip) + parseInt (limit))} > Next </button>
    </div>
  )
}